...
function main() {
    ...
    if (process.argv.length < 3) {
        console.error("Uso: node main.js <gramatica.yalp>");
        process.exit(1);
    }

    const yalpFile = process.argv[2];
    ...
    generatorSLR.generateParserFile(path.join(__dirname, 'parser_slr.py'));
    ...
    generatorLALR.generateParserFile(path.join(__dirname, 'parser_lalr.py'));
